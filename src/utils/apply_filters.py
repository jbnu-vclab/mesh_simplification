...
def apply_qem_and_save(file_path: str, out_path: str, target_percentage: float):
    ...
    ms.meshing_decimation_quadric_edge_collapse(targetperc=target_percentage)
    ms.save_current_mesh(out_path)

def apply_decimation_and_save(file_path: str, out_path: str, target_percentage: float):
    ms = pymeshlab.MeshSet()
    ms.load_new_mesh(file_path)
    ms.meshing_decimation_clustering(threshold=pymeshlab.Percentage(target_percentage*100))
    ms.save_current_mesh(out_path)
...
def apply_normalization_to_models(device, folder_path: str, out_folder_path:str):
    ...
    for file_path in tqdm(gl):
        ...
        apply_normalization_and_save(device, file_path, file_path)

def apply_filter_to_models(folder_path: str, out_folder_path:str, target_percentage: float):
    gl = glob(folder_path + '/*.obj')
    ...
    for file_path in tqdm(gl):
        ...
        # apply_decimation_and_save(file_path, out_path, target_percentage)

    
target_percentage = 0.001
FOLDER_PATH = './data/Thingi10K/original'
OUT_PATH = f'./data/Thingi10K/{target_percentage}'
...
if __name__=='__main__':
    ...
    apply_filter_to_models(FOLDER_PATH, OUT_PATH, target_percentage)
