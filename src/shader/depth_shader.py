...
class SoftDepthShader(ShaderBase):
    ...
    def forward(self, fragments, meshes, **kwargs) -> torch.Tensor:
        ...
        zfar_tensor = torch.ones((N, H, W, 1), device=device) * zfar

        # append extra face for zfar
        dists = torch.cat(
            (fragments.zbuf, zfar_tensor), dim=3
        )
        ...
        depthmap = (probs * dists).sum(dim=3).unsqueeze(3)

        depthmap = depthmap / cameras.zfar
